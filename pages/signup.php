...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(!$daCodiceFiscale) {
        ...
        if(empty($datiDaCodice)) {
            ...
            $cognome = $daCodiceFiscale['cognome'] ?? '';
    ...
    if($status != '') {
        ...
        $stmt = getInstance().getConnection()->prepare($insert_string);
...
<div style="padding: 20px;">
    ...
    <?php if($registratiConCodice) { ?>
        <a href='/signup.php/?conCodiceFiscale=true'>Non hai il codice fiscale?</a>
    <?php } else {?>
        <a href='/signup.php/?conCodiceFiscale=false'>Hai il codice fiscale?</a>
    <?php } ?>
</div>
